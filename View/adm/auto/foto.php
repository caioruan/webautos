    ...
    <body>
                    ...
                    <div class="row">                            
                        ...
                        <?php if (isset($dados['foto'][0])) : ?>
                            <?php foreach ($dados['foto'] as $f) : ?>
                                <?php if ($f->foto_url != "") : ?>
                                    <div class="col-md-2" id="div-foto-<?php echo $f->foto_id; ?>" style="margin-bottom: 35px;text-align: center;">
                                        <a href="<?php echo Router::base(); ?>/midias/fotos/<?php echo $f->foto_url; ?>" data-lightbox= "roadtrip">
                                            <img class="img-thumbnail img-responsive" 
                                                 style="min-height: 155px; max-height: 155px;" 
                                                 src="<?php echo Router::base(); ?>/midias/fotos/<?php echo $f->foto_url; ?>" /> 
                                        </a>
                                        <br/>
                                        <button  class="btn btn-primary btn-xs btn-destaque"
                                                 title="foto principal" 
                                                 data-foto="<?php echo $f->foto_id; ?>" 
                                                 data-auto="<?php echo $f->foto_auto; ?>" 
                                                 data-pos="<?php echo $f->foto_pos; ?>" 
                                                 data-toggle="tooltip"><i class="fa fa-image"></i> 
                                                 PRINCIPAL
                                        </button>

                                        <button  class="btn btn-danger btn-xs  btn-remover-uma-foto"
                                                 title="remover foto" 
                                                 data-foto="<?php echo $f->foto_id; ?>" 
        ...
        <script>
            ...
            $(function () {
                ...
                $('.btn-destaque').on('click',function(){
                    var foto_id = $(this).data('foto')
                    var foto_auto = $(this).data('auto')
                    var url = '<?php echo Router::base(); ?>/auto/foto_destaque/';
                    $.post(url, {foto_id:foto_id, foto_auto:foto_auto}, function (data) {
                           window.location = '<?php echo Router::base(); ?>/auto/foto/'+ foto_auto + '/?ok'
                    });
                })
            });
            lightbox.option({
