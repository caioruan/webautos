...
Class ModelAuto {
    ...
    public function getAll() {
        $sql = "SELECT * FROM auto "
                . "LEFT JOIN versao ON (versao_id = auto_versao) "
    ...
    public function get_all_order($order = 'marca_nome ASC, modelo_nome ASC, versao_nome ASC') {
        $sql = "SELECT * FROM auto "
                ...
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (foto_auto = auto_id) "
                . " GROUP BY auto_id ORDER BY $order, foto_pos ASC ";
        $dados = $this->db->fetchAll($sql);
        return $dados;
    }
    ...
    public function get_filtro($condicao) {
        $sql = "SELECT * FROM auto "
                ...
                . " LEFT JOIN modelo ON (modelo_id = auto_modelo)"
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (auto_id = foto_auto) "
                . " WHERE $condicao "
                . " GROUP BY auto_id ORDER BY versao_modelo ASC, foto_pos ASC";
        $dados = $this->db->fetchAll($sql);
    ...
    public function get_autos_destaque($c = '*') {
        ...
        $sql = "SELECT $c FROM auto "
                ...
                . " LEFT JOIN modelo ON (modelo_id = auto_modelo)"
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (foto_auto = auto_id) WHERE foto_pos = 0 "
                . " GROUP BY auto_id ORDER BY auto_id DESC ";
        $this->db->url = "";
    ...
    public function get_autos_recente($c = '*') {
        $sql = "SELECT $c FROM auto "
                ...
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (foto_auto = auto_id) "
                . " GROUP BY auto_id  ORDER BY auto_id DESC, foto_pos ASC LIMIT 0,4";
        $dados = $this->db->fetchAll($sql);
        return $dados;
    }
    ...
    public function get_autos_popular($c = '*') {
        $sql = "SELECT $c FROM auto "
                ...
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (foto_auto = auto_id) "
                . " GROUP BY auto_id  ORDER BY auto_visita DESC, foto_pos ASC LIMIT 0,5";
        $dados = $this->db->fetchAll($sql);
        return $dados;
    }
    ...
    public function get_autos_topo($c = '*') {
        $sql = "SELECT $c FROM auto "
                ...
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN foto ON (foto_auto = auto_id) "
                . " WHERE auto_topo = 1 AND foto_pos = 0"
    ...
    public function get_all_busca($busca) {
        ...
        $sql = "SELECT * FROM auto "
                ...
                . " LEFT JOIN marca ON (marca_id = auto_marca)  "
                . " LEFT JOIN foto ON (foto_auto = auto_id) "
                . " WHERE versao_nome like'%$busca%' OR modelo_nome like'%$busca%' OR marca_nome like'%$busca%' AND foto_pos = 0 "
                . " GROUP BY auto_id ORDER BY auto_id DESC, foto_pos ASC";
        $this->db->url = "";
    ...
    public function get_fotos($auto_id = null) {
        ...
        $dados = $this->db->fetchAll("SELECT * FROM foto WHERE foto_auto = $this->auto_id ORDER BY foto_pos ASC");
        return $dados;
    }
    ...
    public function get_by_id($auto_id) {
        $sql = "SELECT * FROM auto "
                ...
                . " LEFT JOIN modelo ON (modelo_id = auto_modelo)"
                . " LEFT JOIN marca ON (marca_id = auto_marca) "
                . " LEFT JOIN combustivel ON (combustivel_id = auto_combustivel) "
                . " LEFT JOIN cambio ON (cambio_id = auto_cambio) "
                . " LEFT JOIN cor ON (cor_id = auto_cor) "
                . " LEFT JOIN documentacao ON (documentacao_id = auto_documentacao) "
